...
if 'socket_manager' not in st.session_state:
    ...
    st.session_state.socket_manager.start_socket_server(port=8888, backlog=5)
...
def generate_data():
    ...
    if not selected_device_id or not hasattr(socket_manager, 'latest_real_time_data') or selected_device_id not in socket_manager.latest_real_time_data:
        ...
        data_point = {
            ...
            # 修改状态为五种状态之一
            'status': random.choice(['在床正常', '离床', '呼吸急促', '体动', '呼吸暂停']),
        ...
        try:
            ...
            data_point = {
                ...
                # 修改状态定义，使用label_data[2]
                'status': label_data[2] if label_data and len(label_data) > 2 else '在床正常',
...
def create_reconstruction_loss_chart(title, rl_values, statuses, times):
    ...
    for i, (status, color) in enumerate(STATUS_COLORS.items()):
        x_pos = legend_x + (i % 3) * 90  # 每行3个
        y_pos = legend_y + (i // 3) * 15  # 换行
        svg_content += f"""
    ...
    svg_content += f"""
            ...
            <!-- 当前状态显示 -->
            <text x="{svg_width - 15}" y="35" fill="{STATUS_COLORS.get(statuses[-1], '#4CAF50')}" font-size="10" font-weight="bold" text-anchor="end">{statuses[-1]}</text>
...
if st.session_state.show_data_modal and chart_data:
    with st.expander("📊 数据详情", expanded=True):
        ...
        # 包含所有字段，将anomaly_status改为status
...
if st.session_state.show_chart_modal and len(chart_data) > 1:
    with st.expander("📈 高级趋势图表", expanded=True):
        ...
        fig.update_layout(
            height=800,  # 增加高度适应5个图表
            showlegend=True,
            title_text="生理参数监控趋势（含状态检测）",
...
• <strong>完整监控:</strong> 心率 + 心线原始 + 呼吸率 + 呼吸线原始 + 五状态检测<br>
• <strong>状态分类:</strong> 在床正常🟢 / 离床🟠 / 呼吸急促🔴 / 体动🔵 / 呼吸暂停🟣<br>
...
• <strong>状态检测:</strong> 重构损失值分析 + 实时五状态监控 + 彩色分类显示<br>
• <strong>展开式详情:</strong> 数据表格 + Plotly交互图表（含状态分类）<br>
...
if not st.session_state.monitoring and len(st.session_state.data) == 0:
    ...
    st.markdown('<div style="background-color: #fff3e0; padding: 8px 12px; border-radius: 6px; margin: 8px 0; font-size: 0.85rem;">💡 点击 "📊 数据详情" 展开详细数据表格，点击 "📈 高级图表" 展开交互式分析图表（含状态分类）</div>', unsafe_allow_html=True)