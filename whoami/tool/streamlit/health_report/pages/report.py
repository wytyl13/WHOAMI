...
def validate_device_exists(_sql_provider, device_sn: str) -> bool:
    """验证指定的设备是否存在于数据库中"""
    ...
    try:
        ...
        for record in all_records:
            ...
            if record_device and record_device == device_sn:
                return True
...
def get_time_ranges_for_device(_sql_provider, device_sn: str) -> List[Dict]:
    ...
    try:
        ...
        # 尝试常见的设备字段名进行条件查询
        for field_name in ["device_id", "device_sn"]:
            try:
                records = _sql_provider.get_record_by_condition(
                    condition={field_name: device_sn},
                    fields=None
                )
                if records:
        ...
        if not device_records:
            ...
            if all_records:
                # 客户端过滤
                for record in all_records:
                    for field_name in ["device_id", "device_sn", "deviceSn", "deviceId", "sn"]:
                        if field_name in record and str(record[field_name]) == device_sn:
        ...
        for record in device_records:
            try:
                # 查找开始时间
                start_time = None
                for field_name in ["sleep_start_time", "start_time", "begin_time"]:
                    if field_name in record and record[field_name]:
                ...
                # 查找结束时间
                end_time = None
                for field_name in ["sleep_end_time", "end_time", "finish_time"]:
                    if field_name in record and record[field_name]:
                ...
                # 查找时长
                total_duration = None
                for field_name in ["total_duration", "duration", "total_time"]:
...
def create_sleep_stages_chart(sleep_data: Dict) -> go.Figure:
    ...
    colors = ["#2E8B57", "#4682B4", "#FF8C00", "#FF9800"]
...
def create_behavior_chart(sleep_data: Dict) -> go.Figure:
    ...
    # colors = ["#20B2AA", "#FFD700", "#FF6347", "#FF9800"]
    colors = ["#20B2AA", "#9C27B0", "#FF6347", "#FF9800"]
...
def get_report_params():
    """获取报告参数 - 优先从session_state获取，其次从URL参数获取"""
    # 优先从 session_state 获取（从 real_time_vital.py 跳转传递）
    device_sn = st.session_state.get('report_device_sn')
    device_info = st.session_state.get('report_device_info')
    username = st.session_state.get('report_username')
    start_time = st.session_state.get('report_start_time')
    end_time = st.session_state.get('report_end_time')
    from_real_time_vital = st.session_state.get('from_real_time_vital', False)
    
    # 如果 session_state 中没有，从URL参数获取（兼容原有方式）
    if not device_sn:
        device_sn = st.query_params.get("device_sn")
    
    return device_sn, device_info, username, start_time, end_time, from_real_time_vital

def show_return_button():
    """显示返回按钮"""
    if st.session_state.get('from_real_time_vital', False):
        if st.button("🔙 返回实时监控", use_container_width=True):
            st.switch_page("pages/real_time_vital.py")


def main():
    # 🆕 从URL参数获取device_sn
    # device_sn_param = st.query_params.get("device_sn")
    device_sn_param, device_info, username, start_time, end_time, from_real_time_vital = get_report_params()
    if not device_sn_param:
        # 如果没有提供device_sn参数，显示错误信息
        st.error("❌ 缺少设备参数")
        st.markdown("""
        <div style="background-color: #f8d7da; border: 1px solid #f5c6cb; border-radius: 8px; padding: 1rem; margin: 1rem 0;">
            <h4 style="color: #721c24; margin-top: 0;">页面访问错误</h4>
            <p style="color: #721c24; margin-bottom: 0;">
                此页面需要通过URL参数 <code>device_sn</code> 来指定设备序列号。<br>
                正确的访问方式：<code>?device_sn=设备序列号</code>
            </p>
        </div>
        """, unsafe_allow_html=True)
        
        # 提供一个调试按钮，显示可用的设备
        if st.button("🔍 查看可用设备列表"):
            sql_provider = init_database_connection()
            if sql_provider:
                available_devices = get_all_device_sns(sql_provider)
                if available_devices:
                    st.markdown("**可用的设备序列号：**")
                    for device in available_devices:
                        st.code(f"?device_sn={device}")
                else:
                    st.warning("暂无可用设备")
            else:
                st.error("无法连接到数据库")
    ...
    sleep_data_provider = init_sleep_data_connection()  # 详细数据库
    ...
    # 🆕 验证设备是否存在
    with st.spinner("正在验证设备..."):
        device_exists = validate_device_exists(sql_provider, device_sn_param)
    
    if not device_exists:
        st.error(f"❌ 设备 {device_sn_param} 不存在")
        st.markdown(f"""
        <div style="background-color: #f8d7da; border: 1px solid #f5c6cb; border-radius: 8px; padding: 1rem; margin: 1rem 0;">
            <h4 style="color: #721c24; margin-top: 0;">设备不存在</h4>
            <p style="color: #721c24; margin-bottom: 0;">
                指定的设备序列号 <code>{device_sn_param}</code> 在数据库中不存在。<br>
                请检查设备序列号是否正确。
            </p>
        </div>
        """, unsafe_allow_html=True)
        ...
        if st.button("🔍 查看可用设备列表"):
            available_devices = get_all_device_sns(sql_provider)
            if available_devices:
                st.markdown("**数据库中的设备序列号：**")
                for device in available_devices:
                    st.code(f"?device_sn={device}")
            else:
                st.warning("数据库中暂无设备记录")
        return
    
    # 🆕 显示设备信息头部
    extra_info = ""
    if username:
        extra_info += f" | 用户: {username}"
    if start_time and end_time:
        extra_info += f" | 监控时段: {start_time} ~ {end_time}"

    st.markdown(f"""
    <div class="device-info-header">
        <h2>📱 设备: {device_sn_param}</h2>
        <p>正在分析该设备的睡眠数据...{extra_info}</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col1:
        # 检查是否从实时监控页面跳转而来
        if from_real_time_vital:
            show_return_button()
        else:
            # 兼容原有的URL跳转方式
            return_url = f"http://1.71.15.121:8000/real_time_vital?device_sn={device_sn_param}"
            try:
                st.link_button("← 返回监控页面", url=return_url, type="secondary")
            except:
                st.markdown(f"""
                <a href="{return_url}" target="_self" style="
    ...
    # 🆕 使用传入的device_sn参数（替换原来的设备选择逻辑）
    selected_device = device_sn_param
    ...
    st.markdown(f"""
    <div style="background-color: #e3f2fd; padding: 12px; border-radius: 8px; margin: 1rem 0;">
    ...
    st.markdown("## 📋 睡眠健康报告")
    # 睡眠报告展示
    health_report = sleep_data.get('health_report', '') if sleep_data else ''
    if health_report and health_report.strip():
        with st.expander("📊 点击查看详细睡眠分析报告", expanded=False):
            # 简单的报告格式化
            report_lines = health_report.strip().split('\n')
            ...
            for line in report_lines:
                ...
                if not line:
                    ...
                    if '评分' in content and '分' in content:
                        # 高亮评分
            ...
            # 简单的底部信息
            st.markdown("---")
            st.caption(f"📅 报告基于 {selected_range['start_time'].strftime('%Y-%m-%d')} 的睡眠数据生成")

    else:
        with st.expander("📊 睡眠分析报告", expanded=False):
            if not sleep_data:
                st.error("无法显示报告：睡眠数据获取失败")
            elif 'health_report' not in sleep_data:
                st.warning("该记录暂不支持睡眠分析报告功能")
            else:
                st.info("该记录的睡眠分析报告为空，可能还未生成")
    ...
    with col1:
        ...
        st.markdown(f"""
        <div class="metric-card" style="padding: 1rem; margin: 0.3rem 0;">
            <h4 style="font-size: 0.9rem; margin-bottom: 0.3rem;">🕐 总时长</h4>
            <h2 style="font-size: 1.3rem; margin: 0;">{total_duration_display}</h2>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col2:
        ...
        st.markdown(f"""
        <div class="metric-card" style="padding: 1rem; margin: 0.3rem 0;">
            <h4 style="font-size: 0.9rem; margin-bottom: 0.3rem;">🛏️ 在床时长</h4>
            <h2 style="font-size: 1.3rem; margin: 0;">{in_bed_duration_display}</h2>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col3:
        ...
        st.markdown(f"""
        <div class="metric-card" style="padding: 1rem; margin: 0.3rem 0;">
            <h4 style="font-size: 0.9rem; margin-bottom: 0.3rem;">💓 平均心率</h4>
            <h2 style="font-size: 1.3rem; margin: 0;">{avg_hr:.1f} BPM</h2>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col4:
        ...
        st.markdown(f"""
        <div class="metric-card" style="padding: 1rem; margin: 0.3rem 0;">
            <h4 style="font-size: 0.9rem; margin-bottom: 0.3rem;">🫁 平均呼吸率</h4>
            <h2 style="font-size: 1.3rem; margin: 0;">{avg_br:.1f} /min</h2>
        </div>
        """, unsafe_allow_html=True)
    ...
    with time_col1:
        bed_time_display = format_time_display(sleep_data.get('bed_time'))
        st.markdown(f"""
        <div class="metric-card" style="padding: 1rem; margin: 0.3rem 0;">
            <h4 style="font-size: 0.9rem; margin-bottom: 0.3rem;">🛏️ 上床时间</h4>
            <h2 style="font-size: 1.3rem; margin: 0;">{bed_time_display}</h2>
        </div>
        """, unsafe_allow_html=True)

    with time_col2:
        sleep_time_display = format_time_display(sleep_data.get('sleep_time'))
        st.markdown(f"""
        <div class="metric-card" style="padding: 1rem; margin: 0.3rem 0;">
            <h4 style="font-size: 0.9rem; margin-bottom: 0.3rem;">😴 入睡时间</h4>
            <h2 style="font-size: 1.3rem; margin: 0;">{sleep_time_display}</h2>
        </div>
        """, unsafe_allow_html=True)

    with time_col3:
        wake_time_display = format_time_display(sleep_data.get('wake_time'))
        st.markdown(f"""
        <div class="metric-card" style="padding: 1rem; margin: 0.3rem 0;">
            <h4 style="font-size: 0.9rem; margin-bottom: 0.3rem;">🌅 醒来时间</h4>
            <h2 style="font-size: 1.3rem; margin: 0;">{wake_time_display}</h2>
        </div>
        """, unsafe_allow_html=True)

    with time_col4:
        leave_bed_time_display = format_time_display(sleep_data.get('leave_bed_time'))
        st.markdown(f"""
        <div class="metric-card" style="padding: 1rem; margin: 0.3rem 0;">
            <h4 style="font-size: 0.9rem; margin-bottom: 0.3rem;">🚶 离床时间</h4>
            <h2 style="font-size: 1.3rem; margin: 0;">{leave_bed_time_display}</h2>
        </div>
        """, unsafe_allow_html=True)
