...
def update_device_online_status(device_code, is_online):
    ...
    try:
        ...
        # 首先根据device_code查找设备记录
        device_record = sql_provider.get_record_by_condition(
        ...
        # 获取第一个匹配的记录
        device = device_record[0] if isinstance(device_record, list) else device_record
        update_data = {}
        # 修改状态和更新时间
        update_data["status"] = 'active' if is_online else 'inactive'
        
        # 获取设备ID，然后从更新数据中移除
        device_id = device.pop("id")  # 移除id字段
        
        # 使用record_id更新记录
        result = sql_provider.update_record(device_id, update_data)
        return result
...
def add_device(device_data):
    ...
    try:
        ...
        result = sql_provider.insert_record(new_device)
        return result
    except Exception as e:
