        ...
        <script>
            ...
            class AeroSenseConfigManager {
                ...
                finishConfiguration() {
                    ...
                    const deviceInfo = {
                        device_code: this.deviceInfo.radar_id || this.deviceInfo.mac || this.device?.id || `device_${Date.now()}`,
                    ...
                    try {
                        if (window.Streamlit) {
                            ...
                            this.showMessage('✅ 设备信息已发送至应用', 'success');
                        } else {
                            // 备用方案：使用URL参数
...
def main():
    ...
    # 处理设备保存请求
    if handle_device_save():
        return
    
    # 处理组件返回的数据
    if 'device_to_save' in st.session_state and st.session_state.device_to_save:
        device_info = st.session_state.device_to_save
        
        st.write("### 📋 准备保存到数据库的设备信息")
        ...
        # 清除保存的数据
        del st.session_state.device_to_save
        return
    ...
    try:
        ...
        if received_data and isinstance(received_data, dict):
            if received_data.get("action") == "save_device_info":
                st.session_state.device_to_save = received_data.get("deviceInfo", {})
                st.rerun()
        ...
        with st.form("manual_config"):
            col1, col2 = st.columns(2)
            with col1:
                device_code = st.text_input("设备编号 *", placeholder="设备MAC地址或序列号")
                wifi_name = st.text_input("WiFi名称 *", value="sxkj")
            with col2:
                server_ip = st.text_input("服务器IP", value="1.71.15.121")
                wifi_password = st.text_input("WiFi密码 *", value="88888888", type="password")
            
            if st.form_submit_button("💾 保存设备", use_container_width=True, type="primary"):
                if device_code and wifi_name and wifi_password:
                    device_info = {
                        "device_code": device_code,
                        "wifi_name": wifi_name,
                        "wifi_password": wifi_password,
                    ...
                    connection_manager = DeviceConnectionManager()
                    if connection_manager.save_complete_device_info(device_info):
                        st.success("🎉 设备信息保存成功！")
                        st.balloons()
                    else:
                        st.error("❌ 保存失败")
    ...
    with col3:
        if st.button("🧪 查看已连接设备", use_container_width=True):
            connection_manager = DeviceConnectionManager()
            devices = connection_manager.get_connected_devices()
            if devices:
                st.success(f"找到 {len(devices)} 个已连接设备")
                for device in devices:
                    st.write(f"• {device.get('device_code')} - {device.get('device_name')}")
            else:
                st.info("暂无已连接设备")
