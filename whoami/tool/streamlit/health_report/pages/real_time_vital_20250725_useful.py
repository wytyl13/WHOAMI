...
def get_device_sn():
    """优先从session_state获取device_sn，其次从URL参数获取"""
    try:
        # 优先从 session_state 获取（从 device_data.py 跳转传递）
        device_sn = st.session_state.get('current_device_sn')
        
        if device_sn:
            return device_sn
            
        # 如果 session_state 中没有，从URL参数获取
        if hasattr(st, 'query_params'):
            query_params = st.query_params
            device_sn = query_params.get('device_sn', None)
        else:
...
# 添加返回按钮
if st.session_state.get('from_device_data', False):
    col1, col2 = st.columns([1, 4])
    with col1:
        if st.button("🔙 返回设备数据", use_container_width=True):
...
def get_shared_socket_manager():
    ...
    # device_sn = '13F51B9D10004071111715D807'
    device_sn = None
...
#     selected_device_id = device_sn
    
#     matched_device_key = None
#     if hasattr(socket_manager, 'latest_real_time_data'):
#         for device_key in socket_manager.latest_real_time_data.keys():
#             if device_key.lower() == selected_device_id.lower():  # 👈 关键修改：不区分大小写比较
#                 matched_device_key = device_key
#                 break
...
def generate_data():
    ...
    if hasattr(socket_manager, 'latest_real_time_data'):
        for device_key in socket_manager.latest_real_time_data.keys():
            if device_key == selected_device_id:
                matched_device_key = device_key
                break
    
    # 修改判断条件：使用 matched_device_key is None 而不是 selected_device_id not in
    if (selected_device_id == "未知设备" or 
        not hasattr(socket_manager, 'latest_real_time_data') or 
        matched_device_key is None):  # 👈 这里是关键修改
        ...
        data_point = {
            ...
            'breathing_rate': max(12, min(20, round(br, 1))),
            'status': random.choice(['在床正常', '离床', '呼吸急促', '体动', '呼吸暂停', '清醒', '浅睡眠', '深睡眠']),
            'reconstruction_loss': random.uniform(0.1, 2.0),
            'breath_line': random.uniform(0.8, 1.2),
            'heart_line': random.uniform(0.9, 1.1),
            'device_id': selected_device_id
        }
        ...
        try:
            # 使用匹配到的设备键获取数据
            item = socket_manager.latest_real_time_data[matched_device_key]  # 👈 使用 matched_device_key
            
            # 获取label数据
            label_data = None
            if hasattr(socket_manager, 'latest_real_time_label') and matched_device_key in socket_manager.latest_real_time_label:
                label_data = socket_manager.latest_real_time_label[matched_device_key]  # 👈 使用 matched_device_key
            
            # 从实时数据中获取时间戳并转换为北京时间
            real_timestamp = item[0]
            real_datetime = datetime.fromtimestamp(real_timestamp, tz=beijing_tz)
            
            data_point = {
                'time': real_datetime.strftime('%H:%M:%S'),
                'timestamp': current_time,
                'datetime': real_datetime.strftime('%Y-%m-%d %H:%M:%S'),
                'heart_rate': item[3],
                'breathing_rate': item[1],
                'device_id': selected_device_id,
                'status': label_data[2] if label_data and len(label_data) > 2 else random.choice(['在床正常', '离床', '呼吸急促', '体动', '呼吸暂停', '清醒', '浅睡眠', '深睡眠']),
                'reconstruction_loss': label_data[1] if label_data else 0.0,
                'breath_line': item[2],
                'heart_line': item[4]
            }
            ...
            st.error(f"读取设备 {selected_device_id} 数据出错: {e}")
...
def create_reconstruction_loss_chart(title, rl_values, statuses, times):
    """创建响应式的重构损失值和状态的复合图表"""
    svg_width_px = 700  # 计算用的像素值
    svg_height = 300    # 增加高度适应移动端
    padding_left = 50
    padding_right = 30
    padding_top = 25
    padding_bottom = 100  # 增加底部空间给8个状态的图例
    chart_width = svg_width_px - padding_left - padding_right
    ...
    # 生成SVG - 使用响应式容器
    svg_content = f"""
    <div style="margin: 0.5rem 0; width: 100%;" class="mobile-chart-container">
        <h4 style="color: #9C27B0; margin-bottom: 0.5rem; text-align: center; font-size: 1.2rem;">{title}</h4>
        <svg width="100%" height="{svg_height}" viewBox="0 0 {svg_width_px} {svg_height}" style="border: 1px solid #ddd; background: white; max-width: 100%;">
            <!-- 网格线 -->
            <defs>
    ...
    for tick_y, tick_value in y_ticks:
        svg_content += f"""
            ...
            <text x="{padding_left - 8}" y="{tick_y + 3}" fill="#666" font-size="10" text-anchor="end">{tick_value:.2f}</text>"""
    ...
    legend_y = padding_top + chart_height + 30
    
    # 分两行显示8个状态图例
    for i, (status, color) in enumerate(STATUS_COLORS.items()):
        x_pos = legend_x + (i % 4) * 120  # 每行4个状态，增加间距
        y_pos = legend_y + (i // 4) * 18  # 分两行，增加行距
        svg_content += f"""
            <circle cx="{x_pos}" cy="{y_pos}" r="4" fill="{color}" stroke="white" stroke-width="1"/>
            <text x="{x_pos + 10}" y="{y_pos + 4}" fill="{color}" font-size="10" font-weight="bold">{status}</text>"""
    
    # 当前状态显示
    latest_status = statuses[-1] if statuses else '未知'
    latest_value = rl_values[-1] if rl_values else 0
    svg_content += f"""
            <!-- 当前值显示 -->
            <text x="{svg_width_px - 15}" y="25" fill="#9C27B0" font-size="14" font-weight="bold" text-anchor="end">当前: {latest_value:.3f}</text>
            
            <!-- 当前状态显示 -->
            <text x="{svg_width_px - 15}" y="45" fill="{STATUS_COLORS.get(latest_status, '#4CAF50')}" font-size="12" font-weight="bold" text-anchor="end">{latest_status}</text>
            
            <!-- 范围显示 -->
            <text x="{svg_width_px - 15}" y="65" fill="#999" font-size="10" text-anchor="end">范围: {y_min:.2f} - {y_max:.2f}</text>"""
    
    svg_content += f"""
            <!-- Y轴标签 -->
            <text x="18" y="{padding_top + chart_height/2}" fill="#333" font-size="12" text-anchor="middle" transform="rotate(-90, 18, {padding_top + chart_height/2})">Loss</text>
            
            <!-- X轴标签 -->
            <text x="{padding_left + chart_width/2}" y="{svg_height - 8}" fill="#333" font-size="12" text-anchor="middle">时间</text>
...
def create_chart_html(chart_data):
    ...
    # 响应式图表尺寸
    svg_width_px = 350  # 计算用的像素值
    svg_height = 220    # 增加高度以适应移动端
    padding_left = 45
    padding_right = 30
    padding_top = 25
    padding_bottom = 40
    ...
    def create_chart_svg(title, values, color, default_min, default_max, unit):
        ...
        # 生成SVG - 使用响应式容器
        svg_content = f"""
        <div style="margin: 0.5rem; display: inline-block; vertical-align: top; width: 48%; min-width: 300px;" class="mobile-chart-container">
            <h5 style="color: {color}; margin-bottom: 0.5rem; text-align: center; font-size: 1.1rem;">{title}</h5>
            <svg width="100%" height="{svg_height}" viewBox="0 0 {svg_width_px} {svg_height}" style="border: 1px solid #ddd; background: white; max-width: 100%;">
                <!-- 网格线 -->
                <defs>
        ...
        for tick_y, tick_value in y_ticks:
            svg_content += f"""
                <line x1="{padding_left - 3}" y1="{tick_y}" x2="{padding_left}" y2="{tick_y}" stroke="#333" stroke-width="1"/>
                <text x="{padding_left - 8}" y="{tick_y + 4}" fill="#666" font-size="9" text-anchor="end">{tick_value:.1f}</text>"""
        
        # X轴刻度和标签
        for tick_x, tick_time in x_ticks:
            svg_content += f"""
                <line x1="{tick_x}" y1="{padding_top + chart_height}" x2="{tick_x}" y2="{padding_top + chart_height + 3}" stroke="#333" stroke-width="1"/>
                <text x="{tick_x}" y="{padding_top + chart_height + 15}" fill="#666" font-size="8" text-anchor="middle">{tick_time}</text>"""
        ...
        svg_content += f"""
                ...
                <!-- 当前值显示 -->
                <text x="{svg_width_px - 12}" y="20" fill="{color}" font-size="12" font-weight="bold" text-anchor="end">{current_value:.1f}</text>
...
# 检查设备是否在可用列表中（不区分大小写）
device_available = any(device == device_sn for device in available_devices)  # 👈 关键修改

# 显示设备信息 - 响应式优化
st.markdown(f"""
<div class="status-container" style="background-color: #f0f2f6; padding: 1rem; border-radius: 8px; margin: 1rem 0; word-wrap: break-word; overflow-wrap: break-word;">
🖥️ <strong>监控设备:</strong> {device_sn}<br>
📡 <strong>可用设备数:</strong> {device_count}<br>
🔗 <strong>数据源:</strong> {'实时数据' if device_available else '模拟数据'}  <!-- 👈 使用 device_available -->
</div>
""", unsafe_allow_html=True)
...
if st.session_state.monitor_start_datetime or st.session_state.monitor_end_datetime:
    ...
    st.markdown(f"""
    <div class="status-container" style="background-color: #e8f5e8; padding: 1rem; border-radius: 8px; margin: 1rem 0; word-wrap: break-word; overflow-wrap: break-word;">
    ⏰ <strong>监控时段:</strong><br>{start_info}<br>~ {end_info}
    </div>
    """, unsafe_allow_html=True)
...
if is_mobile:
    # 移动端：分成两行，每行3-4个按钮
    st.markdown('<div class="mobile-button-row">', unsafe_allow_html=True)
    ...
    with col5:
        if st.button("📊 数据详情", use_container_width=True):
    ...
    with col6:
        if st.button("📈 高级图表", use_container_width=True):
            st.session_state.show_chart_modal = not st.session_state.show_chart_modal

    with col7:
        if st.button("📋 查看报告", use_container_width=True):
            # 保存报告相关信息到 session_state
            st.session_state.from_real_time_vital = True
            # st.session_state.report_device_sn = st.session_state.selected_device_id
            st.session_state.report_device_sn = device_sn
            st.session_state.report_device_info = st.session_state.get('return_device_info')
            st.session_state.report_username = st.session_state.get('return_username')
            
            # 如果有监控时间信息，也传递过去
            if st.session_state.monitor_start_datetime:
                st.session_state.report_start_time = st.session_state.monitor_start_datetime
            if st.session_state.monitor_end_datetime:
                st.session_state.report_end_time = st.session_state.monitor_end_datetime
...
st.markdown(f"""
<div class="status-container" style="background-color: #f0f2f6; padding: 1rem; border-radius: 8px; margin: 1rem 0; word-wrap: break-word; overflow-wrap: break-word;">
📊 <strong>当前数据点:</strong> {len(st.session_state.data)}/{st.session_state.max_points}<br>
⏱️ <strong>更新频率:</strong> 1秒/次<br>
...
with elements("realtime_monitor"):
    ...
    if chart_data:
        ...
        with mui.Grid(container=True, spacing=1.5):
            ...
            with mui.Grid(item=True, xs=6, sm=4, md=2.4):
                with mui.Card(elevation=2, sx={"height": "100px"}):  # 增加高度适应移动端
                    with mui.CardContent(sx={"padding": "15px", "&:last-child": {"paddingBottom": "15px"}}):
                        mui.Typography("心率", variant="subtitle2", color="textSecondary", sx={"fontSize": "1rem"})
                        mui.Typography(
                            f"{latest['heart_rate']} BPM", 
                            variant="h6", 
                            color="error",
                            sx={"fontWeight": "bold", "margin": "6px 0", "fontSize": "1.2rem"}
                        )
                        if len(chart_data) > 1:
            ...
            # with mui.Grid(item=True, xs=2.4):
            with mui.Grid(item=True, xs=6, sm=4, md=2.4):
                with mui.Card(elevation=2, sx={"height": "100px"}):
                    with mui.CardContent(sx={"padding": "15px", "&:last-child": {"paddingBottom": "15px"}}):
                        mui.Typography("心线原始", variant="subtitle2", color="textSecondary", sx={"fontSize": "1rem"})
                        mui.Typography(
                            f"{latest['heart_line']:.3f}", 
                            variant="h6", 
                            color="error",
                            sx={"fontWeight": "bold", "margin": "6px 0", "fontSize": "1.2rem", "opacity": 0.8}
                        )
                        mui.Typography(
                            "原始信号",
                            variant="caption",
            ...
            with mui.Grid(item=True, xs=6, sm=4, md=2.4):
                with mui.Card(elevation=2, sx={"height": "100px"}):
                    with mui.CardContent(sx={"padding": "15px", "&:last-child": {"paddingBottom": "15px"}}):
                        mui.Typography("呼吸率", variant="subtitle2", color="textSecondary", sx={"fontSize": "1rem"})
                        mui.Typography(
            ...
            with mui.Grid(item=True, xs=6, sm=4, md=2.4):
                with mui.Card(elevation=2, sx={"height": "100px"}):
                    with mui.CardContent(sx={"padding": "15px", "&:last-child": {"paddingBottom": "15px"}}):
                        mui.Typography("呼吸线原始", variant="subtitle2", color="textSecondary", sx={"fontSize": "1rem"})
                        mui.Typography(
            ...
            with mui.Grid(item=True, xs=6, sm=4, md=2.4):
                with mui.Card(elevation=2, sx={"height": "100px"}):
                    with mui.CardContent(sx={"padding": "15px", "&:last-child": {"paddingBottom": "15px"}}):
                        mui.Typography("状态监测", variant="subtitle2", color="textSecondary", sx={"fontSize": "1rem"})
                        ...
                        mui.Typography(
                            ...
                            sx={"fontWeight": "bold", "margin": "6px 0", "fontSize": "1.1rem"}
                        )
...
if st.session_state.show_data_modal and chart_data:
    with st.expander("📊 数据详情", expanded=True):
        ...
        # 创建数据表格
        df_display = pd.DataFrame(chart_data[-15:])  # 显示最近15条
...
# 底部说明 - 响应式样式
st.markdown(f"""
<div style="border-top: 1px solid #e6e6e6; margin-top: 1.5rem; padding-top: 1rem;">
<details>
<summary style="cursor: pointer; font-weight: bold; color: #666; font-size: 1.1rem;">✨ 功能特点</summary>
<div style="margin-top: 1rem; color: #555; line-height: 1.8;">
...
• <strong>响应式设计:</strong> 完美适配手机和桌面端，字体大小优化<br>
...
if not st.session_state.monitoring and len(st.session_state.data) == 0:
    st.markdown('<div style="background-color: #e3f2fd; padding: 1rem; border-radius: 8px; margin: 1rem 0; font-size: 1.1rem; text-align: center;">👆 点击"开始监控"开始实时数据累加展示（自动记录时间范围）</div>', unsafe_allow_html=True)
elif len(st.session_state.data) > 0:
    st.markdown(f'<div style="background-color: #e8f5e8; padding: 1rem; border-radius: 8px; margin: 1rem 0; font-size: 1.1rem; text-align: center;">💾 已累积 {len(st.session_state.data)} 个数据点，监控状态: {"🟢 运行中" if st.session_state.monitoring else "⏸️ 已暂停"}</div>', unsafe_allow_html=True)
    st.markdown('<div style="background-color: #fff3e0; padding: 1rem; border-radius: 8px; margin: 1rem 0; font-size: 1rem; text-align: center;">💡 点击 "📊 数据详情" 展开详细数据表格，点击 "📈 高级图表" 展开交互式分析图表，停止监控后可查看生成的健康报告</div>', unsafe_allow_html=True)