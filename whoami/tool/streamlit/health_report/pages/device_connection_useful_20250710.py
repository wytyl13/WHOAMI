...
def start_api_server():
    ...
    try:
        import requests
        response = requests.get('http://localhost:8889/docs', timeout=2)
    ...
    for i in range(max_retries):
        try:
            import requests
            response = requests.get('http://localhost:8889/docs', timeout=1)
            if response.status_code == 200:
                print(f"✅ API服务启动成功 (第{i+1}次尝试)")
                return "http://localhost:8889"
        except:
            time.sleep(1)
    
    print("⚠️ API服务启动超时，但继续运行")
    return "http://localhost:8889"
            ...
            class AeroSenseConfigManager {
                ...
                finishConfiguration() {
                    ...
                    const deviceInfo = {
                        ...
                        server_ip: document.getElementById('server-ip').value.trim() || '1.71.15.121',
                        server_port: document.getElementById('server-port').value.trim() || '8888',
                        working_distance: parseFloat(document.getElementById('working-distance')?.value || '1.5'),
                        report_interval: parseInt(document.getElementById('report-interval')?.value || '1'), // 添加这行
                    ...
                    // 调用FastAPI接口保存设备信息
                    fetch('http://localhost:8889/api/save_device', {
                        method: 'POST',
                        headers: {
                    ...
                    .then(data => {
                        ...
                        if (data.success) {
                            this.showMessage(`✅ 设备信息已成功保存! 设备编号: ${data.device_code}`, 'success');
                            
                            // 显示成功消息
                            const messagesDiv = document.getElementById('messages');
                            const successDiv = document.createElement('div');
                            successDiv.className = 'status-box status-success';
                            successDiv.innerHTML = `
                                <strong>🎉 配置成功!</strong>
                                <p>设备已成功添加到数据库。您可以继续配置其他设备或返回设备管理页面。</p>
                                <div style="margin-top: 15px;">
                                    <button class="btn btn-primary" onclick="window.location.reload()">🔄 继续添加设备</button>
                                    <button class="btn btn-success" onclick="window.parent.postMessage({action: 'redirect', url: '/pages/user_dashboard.py'}, '*')" style="margin-left: 10px;">🏠 返回设备管理</button>
                                </div>
                            `;
                    ...
                    .catch(error => {
                        ...
                        errorDiv.className = 'status-box status-error';
                        errorDiv.innerHTML = `
                            <strong>❌ 保存失败!</strong>
                            <p>错误详情: ${error.message}</p>
                            <div style="margin-top: 10px;">
                                <button class="btn btn-warning" onclick="configManager.finishConfiguration()">🔄 重试</button>
                                <button class="btn btn-secondary" onclick="this.parentElement.remove()" style="margin-left: 10px;">❌ 关闭</button>
                            </div>
                        `;
...
def main():
    ...
    if 'api_url' not in st.session_state:
        st.session_state.api_url = start_api_server()
        
    # 检查API服务状态
    try:
        import requests
        response = requests.get('http://localhost:8889/api/health', timeout=2)
        if response.status_code == 200:
    ...
    with col3:
        if st.button("🧪 测试API连接", use_container_width=True):
            try:
                import requests
                response = requests.get('http://localhost:8889/api/health', timeout=3)
                if response.status_code == 200:
                    data = response.json()
