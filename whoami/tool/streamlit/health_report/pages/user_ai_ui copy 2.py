...
planning_agent_user = PlanningAgentCommunityAiUser(
    tools=[direct_llm_tool_user, handle_tongzhi_tonggao, weather_api, water_machine_api, google_search_tool, health_report_user], 
    enhance_llm=enhance_qwen_admin
)
...
def get_user_info(username: str) -> Optional[cl.User]:
    """获取用户信息"""
    try:
        ...
        if result and len(result) > 0:
            ...
            return cl.User(
                identifier=username, 
                metadata={"role": user_data["role"], "community": user_data["community"]},
            )
        return None
...
@app.middleware("http")
async def capture_username(request: Request, call_next):
    """中间件：捕获URL中的username参数"""
    username = request.query_params.get('username')
    if username:
        # 将username存储到请求状态中
        request.state.captured_username = username
        print(f"中间件捕获到username: {username}")
...
async def main(message: cl.Message):
    ...
    # 如果用户不存在，使用默认用户信息
    if not user:
        print("警告: 用户会话中没有用户信息，使用默认用户")
        user = cl.User(
            identifier="guest", 
            metadata={"role": "user", "community": "默认社区"}
        )
    ...
    if user and hasattr(user, 'metadata') and user.metadata:
        community = user.metadata.get("community", "默认社区")
        role = user.metadata.get("role", "user")
    
    print(f"处理消息 - 用户: {user.identifier if user else 'None'}, 社区: {community}, 角色: {role}")
    ...
    if message.elements:
        ...
        print(f"处理后的chat_history: ------------------------------- {chat_history}")
...
async def start():
    ...
    # 方法1: 从chainlit context获取
    try:
        from chainlit.context import context
        debug_info.append("✓ 成功导入 chainlit.context")
        
        if hasattr(context, 'session') and hasattr(context.session, 'request'):
            request = context.session.request
            username = request.query_params.get('username')
            if username:
                debug_info.append(f"✓ 从URL参数获取username: {username}")
            else:
                debug_info.append("⚠️ URL中没有username参数")
                # 打印所有查询参数进行调试
                all_params = dict(request.query_params)
                debug_info.append(f"所有URL参数: {all_params}")
        else:
            debug_info.append("✗ context.session.request 不存在")
    except Exception as e:
        debug_info.append(f"✗ 从context获取URL参数失败: {str(e)}")
    
    # 方法2: 从会话状态获取（如果通过中间件设置了）
    if not username:
        try:
            # 尝试从会话中获取之前存储的username
            stored_username = cl.user_session.get("captured_username")
            if stored_username:
                username = stored_username
                debug_info.append(f"✓ 从会话状态获取username: {username}")
        except:
            pass
    
    # 使用默认用户名
    if not username:
        username = "testuser"  # 改为testuser，匹配你的Streamlit传递的参数
        debug_info.append(f"✓ 使用默认用户名: {username}")
    
    print(f"最终确定的用户名: {username}")
    
    # 根据username获取用户信息
    user = get_user_info(username)
    
    # 如果数据库中没有该用户，创建一个基本的用户对象
    if not user:
        user = cl.User(
            identifier=username, 
            metadata={"role": "user", "community": "默认社区"}
        )
        debug_info.append(f"✓ 创建默认用户对象: {username}")
    else:
        debug_info.append(f"✓ 从数据库获取用户信息: {username}")
    
    # 将用户信息设置到会话中
    cl.user_session.set("user", user)
    cl.user_session.set("captured_username", username)  # 额外存储一份
    ...
    # 获取用户信息
    user_role = user.metadata.get("role", "user") if user.metadata else "user"
    community = user.metadata.get("community", "默认社区") if user.metadata else "默认社区"
    
    debug_info.append(f"✓ 用户角色: {user_role}")
    debug_info.append(f"✓ 用户社区: {community}")
    ...
    try:
        ...
        debug_info.append("✓ 成功获取通告信息")
    except Exception as e:
        debug_info.append(f"⚠️ 获取通告信息失败: {str(e)}")
    
    # 构建欢迎消息
    if user_role == "admin":
        message_content = f"""
欢迎，{community}超管 **{username}**！我是你的社区智能体助手，我可以帮你发布时讯消息、通告等其它操作！
...
提示：首次使用语音功能时，浏览器可能会询问麦克风权限，请点击"允许"。
        ...
        message_content = f"""
欢迎，尊敬的{community}用户 **{username}**！我是你的社区智能体助手，你可以咨询我任何问题！
