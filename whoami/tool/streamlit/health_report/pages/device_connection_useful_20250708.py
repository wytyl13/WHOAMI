        ...
        <script>
            class AeroSenseConfigManager {
                ...
                async getDeviceId() {
                    ...
                    try {
                        ...
                        if (this.deviceInfo.mac) {
                            this.showMessage(`✅ 设备ID (MAC): ${this.deviceInfo.mac}`, 'success');
                            this.notifyParent({
                                type: 'device_id_obtained',
                                device_id: this.deviceInfo.mac
                            });
                            if (this.deviceInfo.radar_id) {
                                this.notifyParent({
                                    type: 'device_sn_obtained', 
                                    device_sn: this.deviceInfo.radar_id
                                });
                            }
                        } 
                        else {
                            this.showMessage('⚠️ 未能获取到设备ID，请重试', 'warning');
                        }
                ...
                createRadarIdCommand() {
                    ...
                    console.log(`📊 构造Radar ID查询命令: 0x0410`);
                    return command;
                }
                ...
                updateServerStatus(serverIP, serverPort, status = '配网完成') {
                    ...
                    if (serverStatusText) {
                        serverStatusText.textContent = '设备将在断开蓝牙后自动连接WiFi并向服务器发送数据';
                    }
                }
                ...
                async startConfiguration() {
                    ...
                    try {
                        ...
                        await this.delay(5000); // 等待设备连接WiFi和服务器

                        // 步骤4: 验证配置
                        this.updateProgress(85, '🔍 等待设备连接服务器...');
                        this.showMessage(`📡 设备将连接到服务器 ${serverIP}:${serverPort}`, 'info');
                        this.showMessage('💡 请观察设备LED：蓝灯常亮=成功，红灯常亮=连接中', 'info');
                ...
                handleBinaryResponse(bytes) {
                    try {
                        ...
                        // 处理Get Radar ID响应 (0x0410)
                        if (funcTag === 0x0410) {
                            // Radar ID数据从第16字节开始，长度为13字节
                            ...
                            console.log(`✅ 解析到Radar ID: ${radarId}`);
                        }
                ...
                /**
                * 构造设置数据上报间隔命令 (协议 0x03e9)
                */
                createReportIntervalCommand(intervalMs = 1000) {
                    // 转换为50ms单位 (1000ms = 20 * 50ms)
                    const interval = Math.floor(intervalMs / 50);
                    
                    // 构造16进制命令字符串
                    const command = String.fromCharCode(
                        0x13, 0x01, 0x01, 0x01, 0x00, 0x00, 0x00, 0x01, // 协议头
                        0x00, 0x0A, 0x00, 0x00, 0x00, 0x06, 0x03, 0xE9, // 功能码 0x03e9
                        (interval >> 24) & 0xFF, (interval >> 16) & 0xFF, 
                        (interval >> 8) & 0xFF, interval & 0xFF          // 间隔参数
                    );
                    
                    console.log(`📊 设置数据上报间隔: ${intervalMs}ms (${interval} * 50ms)`);
                    return command;
                }
