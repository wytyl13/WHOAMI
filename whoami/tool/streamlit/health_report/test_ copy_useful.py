...
def create_chart_html_bake(chart_data):
    """创建带坐标轴的紧凑SVG图表"""
    ...
    def create_chart_svg(title, values, color, y_min, y_max, unit):
        ...
        for tick_y, tick_value in y_ticks:
            svg_content += f"""
                ...
                <text x="{padding_left - 8}" y="{tick_y + 3}" fill="#666" font-size="9" text-anchor="end">{tick_value:.0f}</text>"""
    ...
    if chart_data:
        ...
        with mui.Grid(container=True, spacing=1.5):
            ...
            with mui.Grid(item=True, xs=3):
                with mui.Card(elevation=2, sx={"height": "120px"}):
                    with mui.CardContent(sx={"padding": "12px", "&:last-child": {"paddingBottom": "12px"}}):
                        mui.Typography("设备", variant="subtitle2", color="textSecondary", sx={"fontSize": "0.8rem"})
                        mui.Typography(
...
if st.session_state.show_data_modal and chart_data:
    with st.expander("📊 数据详情", expanded=True):
        ...
        if 'device_id' in df_display.columns:
            df_display = df_display[['time', 'heart_rate', 'breathing_rate', 'device_id', '状态']]
            df_display.columns = ['时间', '心率 (BPM)', '呼吸率 (/min)', '设备ID', '状态']
        else:
            df_display = df_display[['time', 'heart_rate', 'breathing_rate', '状态']]
            df_display.columns = ['时间', '心率 (BPM)', '呼吸率 (/min)', '状态']
...
#         st.dataframe(df_display, use_container_width=True, height=300)  # 限制高度
...
if st.session_state.show_chart_modal and len(chart_data) > 1:
    with st.expander("📈 高级趋势图表", expanded=True):
        ...
        fig = make_subplots(
            ...
            vertical_spacing=0.2  # 进一步增加垂直间距
        )
        ...
        fig.update_layout(
            ...
            showlegend=False,
            title_text="生理参数监控趋势",
            margin=dict(l=40, r=40, t=50, b=40)  # 增加顶部边距
        )
