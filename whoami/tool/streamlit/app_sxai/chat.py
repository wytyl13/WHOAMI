...
def initialize_chat_state():
    ...
    if "last_assistant_message_index" not in st.session_state:
        st.session_state.last_assistant_message_index = -1
...
def apply_chat_styles():
    ...
    <style>
        ...
        .stChatMessage {
            ...
            position: relative !important;  /* 为音频图标定位做准备 */
        }
        ...
        /* Show audio player only when needed */
        .hidden-player {
...
def render_chat_interface():
                ...
                if utils.is_valid_message(message):
                    with st.chat_message(message["role"]):
                        ...
                        if (message["role"] == "assistant" and 
                            st.session_state.audio_ready and 
                            i == st.session_state.last_assistant_message_index):
                            
                            # 自定义样式的音频播放器
                            # 移除display:none样式，显示原生音频播放器
                            audio_html = f"""
                            <div style="margin-top: 10px;">
                                <audio id="audio-{i}" src="{st.session_state.current_audio_url}" controls></audio>
                            </div>
                            """
                            st.markdown(audio_html, unsafe_allow_html=True)
                            print(f"已渲染音频按钮，音频URL: {st.session_state.current_audio_url}")
                else:
    ...
    # 检查是否有新的音频准备就绪，如果有则重新加载页面
    if st.session_state.get("_tts_ready_to_display", False):
        st.session_state._tts_ready_to_display = False
        print("音频已准备好，刷新页面显示...")
        st.rerun()
...
def handle_new_message(prompt: str):
    ...
    # 记录最后一条助手消息的索引
    st.session_state.last_assistant_message_index = len(st.session_state.messages) - 1
    
    # 调试信息
    print(f"AI回答: {assistant_response[:50]}...")
    
    # 在后台生成TTS，不阻塞UI
    audio_url = api.generate_tts(assistant_response)
    if audio_url:
        print(f"成功获取音频URL: {audio_url}")
        # 设置音频状态，但不立即重载页面
        st.session_state.current_audio_url = audio_url
        st.session_state.audio_ready = True
        st.session_state._tts_ready_to_display = True
        # 重载页面以显示音频按钮
        st.rerun()
    else:
        print("无法获取音频URL")
