...
def create_heart_line_chart(detail_data: List[Dict]) -> go.Figure:
    ...
    heart_bpms = [d["heart_bpm"] for d in detail_data if d["heart_bpm"] is not None]
    
    if not heart_lines and not heart_bpms:
        fig = go.Figure()
        fig.add_annotation(
            text="暂无有效心率数据",
            x=0.5, y=0.5,
    ...
    # 心率BPM（如果有的话，作为第二条线）
    if heart_bpms:
        heart_bpm_times = [d["datetime"] for d in detail_data if d["heart_bpm"] is not None]
        fig.add_trace(go.Scatter(
            x=heart_bpm_times,
            y=heart_bpms,
            mode='lines+markers',
    ...
    # 如果有heart_bpm数据，添加第二个y轴
    if heart_bpms:
        fig.update_layout(
            yaxis2=dict(
                title={'text': "BPM", 'font': {'size': 12}},
                overlaying='y',
                side='right',
                tickfont={'size': 10}
            )
