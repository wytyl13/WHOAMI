...
with elements("realtime_monitor"):
    ...
    if chart_data:
        ...
        # 顶部指标卡片 - 增加breath_line和heart_line显示
...
if st.session_state.show_data_modal and chart_data:
    with st.expander("📊 数据详情", expanded=True):
        ...
        if 'device_id' in df_display.columns:
            df_display = df_display[['time', 'heart_rate', 'heart_line', 'breathing_rate', 'breath_line', 'reconstruction_loss', 'anomaly_status', 'device_id', '状态']]
            df_display.columns = ['时间', '心率(BPM)', '心线原始', '呼吸率(/min)', '呼吸线原始', '重构损失', '异常状态', '设备ID', '生理状态']
        else:
            df_display = df_display[['time', 'heart_rate', 'heart_line', 'breathing_rate', 'breath_line', 'reconstruction_loss', 'anomaly_status', '状态']]
            df_display.columns = ['时间', '心率(BPM)', '心线原始', '呼吸率(/min)', '呼吸线原始', '重构损失', '异常状态', '生理状态']
...
• <strong>完整监控:</strong> 心率 + 心线原始 + 呼吸率 + 呼吸线原始 + 异常检测<br>
...
• <strong>展开式详情:</strong> 数据表格 + Plotly交互图表（含原始信号）<br>
...
if not st.session_state.monitoring and len(st.session_state.data) == 0:
    ...
    st.markdown('<div style="background-color: #fff3e0; padding: 8px 12px; border-radius: 6px; margin: 8px 0; font-size: 0.85rem;">💡 点击 "📊 数据详情" 展开详细数据表格，点击 "📈 高级图表" 展开交互式分析图表（含原始信号）</div>', unsafe_allow_html=True)