...
class VideoStreamDetector(BaseModel, ABC):
    ...
    def process(self, stop_event: threading.Event):
        ...
        def process_single_connection():
            ...
            try:
                if not self._handle_connection_status(cap, topic, real_topic_list):
                    return False
                
                while cap.isOpened() and not stop_event.is_set():
                    if not self._process_frame(cap, topic, real_topic_list, last_sample_time):
                        return False
    ...
    def _handle_connection_status(self, cap: cv2.VideoCapture, topic: str, real_topic_list: List[str]) -> bool:
        """处理连接状态"""
        if not cap.isOpened():
            self.logger.error(f"Failed to open stream_url: {self.stream_url}")
    ...
    def _process_frame(self, cap: cv2.VideoCapture, topic: str, 
                      real_topic_list: List[str], last_sample_time: float) -> bool:
        """处理单帧图像"""
        ret, frame = cap.read()
        
        try:
            if not self._validate_frame(ret, frame, topic, real_topic_list):
                return False
                
            current_time = time.perf_counter()
            if current_time - last_sample_time >= (10 / 100):  # 200毫秒采样
                last_sample_time = current_time
                task_id = self.device_sn + topic
                self.logger.info(f"{task_id} running!")
                
                return self._process_detection(frame, topic, real_topic_list)
    ...
    def _validate_frame(self, ret: bool, frame: Optional[np.ndarray], 
                       topic: str, real_topic_list: List[str]) -> bool:
        """验证帧的有效性"""
        if not ret or frame is None or frame.size == 0:
            self.logger.error(f"Failed to read frame from stream url: {self.stream_url}")
            if topic in real_topic_list:
                real_topic_list.remove(topic)
            self.update_sql_video_stream_status(real_topic_list)
            return False
    ...
    def _process_detection(self, frame: np.ndarray, topic: str, real_topic_list: List[str]) -> bool:
        """处理检测结果"""
        try:
