...
class RealTimeStateMonitor:
    ...
    def __init__(self,
        ...
        # 新增：原始数值历史缓冲区（用于8秒稳定性判断）
        self.value_history = []                   # [(timestamp, value), ...]
        self.value_window_duration = 8.0          # 8秒窗口
        self.stability_threshold = 0.1            # 稳定性阈值（前后变动不超过0.1）
        
    def update(self, value: float, timestamp: Optional[float] = None, body_move_energy: int = 0) -> Tuple[str, str, Optional[PeakEvent]]:
        """
        更新状态监测器
    ...
    def get_debug_info(self) -> dict:
        ...
        debug_info = {
            ...
            'current_body_move_energy': self.current_body_move_energy,  # 添加当前state值到调试信息
            'state_history_length': len(self.state_history),   # state历史长度
            'value_history_length': len(self.value_history),   # value历史长度（新增）
            'recent_state_stats': self._get_recent_state_stats(),  # 最近的state统计
            'apnea_stability_check': self._is_stable_in_apnea_range(self.last_update_time) if self.last_update_time else False  # 呼吸暂停稳定性检查（新增）
        }
