...
class StateSmoother:
    ...
    def __init__(self, 
                 window_size: int = 6,           # 滑动窗口大小（增大以适应60秒要求）
                 min_state_duration: float = 30.0,  # 正常状态最小持续时间60秒
    ...
    def _handle_normal_state_change(self, new_normal_state: str, timestamp: float) -> str:
        ...
        if smoothed_by_voting == new_normal_state or smoothed_by_duration == new_normal_state:
    ...
    def _smooth_by_voting(self) -> str:
        ...
        if most_common_count / total_count >= 0.6:
...
def add_data_point_with_smoothing(self, 
    ...
    if not hasattr(self, 'state_smoother'):
        self.state_smoother = StateSmoother(
            window_size=6,          # 增大窗口
            min_state_duration=30.0, # 60秒最小持续时间
