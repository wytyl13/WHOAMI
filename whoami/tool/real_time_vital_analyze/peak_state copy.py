...
class RealTimeStateMonitor:
    ...
    def update(self, value: float, timestamp: Optional[float] = None) -> Tuple[str, str, Optional[PeakEvent]]:
        """
        更新状态监测器
        ...
        # 2. 峰值状态检测
        peak_state, peak_event = self._update_peak_detection(value, timestamp)
    ...
    def _update_peak_detection(self, value: float, timestamp: float) -> Tuple[PeakState, Optional[PeakEvent]]:
        """峰值检测更新（复用之前的逻辑）"""
        # 判断是否应该激活峰值检测
        ...
        if not should_activate:
            ...
            if self.current_peak_state in [PeakState.RISING, PeakState.PEAK, PeakState.FALLING]:
                event = self._force_complete_peak(timestamp)
        ...
        # 状态转移
        new_state, event = self._update_peak_state(value, timestamp)
    ...
    def _classify_overall_state(self, value: float, validated_base_state: str, 
        ...
        if peak_event and peak_event.event_type == "peak_completed":
            if peak_event.peak_value > 100:
                return "呼吸急促"
            elif peak_event.peak_value >= 10:
                return "体动"
        ...
        if recent_peaks:
            ...
            if time_since_peak <= 5.0:
                if latest_peak.peak_value > 100:
                    return "呼吸急促"
                elif latest_peak.peak_value >= 10:
                    return "体动"
    ...
    # 以下是峰值检测的内部方法（与之前的FilteredPeakDetector相同）
    ...
    def _update_peak_state(self, value: float, timestamp: float) -> Tuple[PeakState, Optional[PeakEvent]]:
    ...
    def _force_complete_peak(self, timestamp: float) -> Optional[PeakEvent]:
