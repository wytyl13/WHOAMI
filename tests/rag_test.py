...
from whoami.tool.llm_application.chat_sys import ChatSys
...
llm=Ollama(model="qwen2.7-7b-Instruction-sx-8epochs-258:latest", request_timeout=360.0, temperature=0.1)
...
def test_rag():
    ...
    async def chat_health_report(request_data: RequestDataChat):
        ...
        if not messages:
            result = sql_provider.get_record_by_condition(condition={"user_id": user_id})
            if result:
